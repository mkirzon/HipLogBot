import logging
from src.models.record import Activity, Record, Pain
...
class DailyLog:
  ...
  def from_dict(cls, input_dict):
    ...
    if input_dict.get('activities'):
      for activity_name, record in input_dict['activities'].items():
        res = instance.add_activity(**record)
  ...
  def add_activity(self, name: str, **attributes):
    ...
    if name in self._activities:
      logging.info(f"Activity '{name}' already exists for {self._date}. Overwriting previous entry.")
      res = "update"
    else: 
      res = "new"

    # Create/update activity with the provided attributes and add it to the records
  ...
  def delete_activity(self, name: str):
      ...
      if name in self._activities:
          ...
          logging.info(f"Activity '{name}' doesn't exist for {self._date}. Nothing to delete.")
          return False
